...
private:
	...
	Matrix prepareRotation(int w, int h);
	Matrix prepareScale(int w, int h); 
	Matrix prepareTranslation(); 
	void rotatedText(wxDC *dc, Matrix transformation, wxString text, double x, double y, int w, int h);
	void line2d(Matrix transformation, double x1, double y1, double x2, double y2, int w, int h, wxDC *dc);
