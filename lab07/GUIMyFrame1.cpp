...
MyFrame1( parent )
{
 m_tb_pokaz_punkty->SetLabel(_(L"Show points"));
 sbSizer2->GetStaticBox()->SetLabel(_(L"Choose function"));
...
m_st_liczba_poziomic->SetLabel("Levels: "+ wxString::Format(wxT("%i"), NoLevels));
DrawMap(NoPoints, FunctionData, ContourFlag, ColoringType, NoLevels, ShowPoints);
Repaint();
}
